...
def apply_AWGN_with_pitch_shift(waveform, sr, bits=16, snr_min=15, snr_max=30, pitch_steps=(-2, 2)):
    ...
    pitch_shift_amount = np.random.uniform(*pitch_steps)
    augmented_waveform = librosa.effects.pitch_shift(augmented_waveform, sr=sr, n_steps=pitch_shift_amount)

    augmented_waveform = augmented_waveform.astype(np.float32)

    return augmented_waveform

def detect_speech(waveform, start_time, end_time, sr, frame_length=2048, hop_length=512, threshold_energy=0.2):
